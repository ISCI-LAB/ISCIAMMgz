...
namespace gazebo
{
  class PlannarMover_unitCylinder : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&PlannarMover_unitCylinder::OnUpdate, this));
      
      this->old_secs =ros::Time::now().toSec();
      
      // Create a topic name
      std::string plannar_pos_topicName = "/obsvel_unitCylinder";
      ...
      if (!ros::isInitialized())
      {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "plannar_unitCylinder",
            ros::init_options::NoSigintHandler);
      }
         
      // Create our ROS node. This acts in a similar manner to
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("plannar_unitCylinder_rosnode"));
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::Twist>(
            ...
            1,
            boost::bind(&PlannarMover_unitCylinder::OnRosMsg_Pos, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&PlannarMover_unitCylinder::QueueThread, this));

      ROS_WARN("Loaded PlannarMover_unitCylinder Plugin with parent...%s, only X Axis Freq Supported in this V-1.0", this->model->GetName().c_str());
    ...
    double x_axis_pos = 20.0;
    // Magnitude of the Oscilations
    double y_axis_pos = 20.0;
  ...
  GZ_REGISTER_MODEL_PLUGIN(PlannarMover_unitCylinder)
}